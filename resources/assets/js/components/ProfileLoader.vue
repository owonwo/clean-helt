<template>
	<section>
		<div v-preload v-if="isLoading"
			class="block is-rounded mx-15 my-5 mb-0" style="height:10px; border-radius: 0"/>
...
export default {
    ...
    methods: {
        fetchData() {
        	this.isLoading = true;
        	!this.testChCode(this.$props.chcode) ||
        	this.$http.get(this.getUrl()).then((res) => {
        		this.user = this.personalify(res.data.data);
